...
export async function handleAdminAgent(body: AgentRequestBody, res: ServerResponse) {
  const prepared = await prepareConversation("admin", body)
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Unknown admin agent error"
