...
![01.3-Product Detail](Sketches/01.3-product-detail.excalidraw)
...
Bottom sheet overlay that slides up from the bottom of the viewport. Covers approximately 90% of the screen height. The product cards (01.2) remain visible behind a darkened/blurred scrim at the top.
...
**Responsive behavior:**
- **Mobile (< 768px):** Full-width bottom sheet, 90% viewport height, content scrolls vertically within the sheet
- **Tablet (768px–1024px):** Bottom sheet max-width 600px, centered horizontally
- **Desktop (>= 1024px):** Side panel or centered modal (max-width 520px) — the bottom sheet pattern is mobile-native; on desktop, consider a side drawer or centered overlay
...
| Property | Value |
|----------|-------|
| Component | Drag Handle Bar |
| Behavior | Swipe down to dismiss overlay → return to 01.2-Product Cards |
| Visual | Centered horizontal bar, 40px wide, 4px tall, border-default color, rounded |
...
| Property | Value |
|----------|-------|
| Component | EU Label Slider (expandable) |
...
| NO | "Gjennomsnittlig drivstofforbruk — du merker ingen forskjell pa bensinkostnaden." |
...
- **Bottom sheet behavior:** Use a spring-based animation for open/close. Overlay slides up from bottom with velocity-based dismiss (fast swipe down = close, slow drag = snap back or close depending on threshold at ~40% height).
- **Scrim backdrop:** Cards behind the overlay get `backdrop-filter: blur(4px)` plus a dark overlay at 40% opacity. Tapping the scrim closes the overlay.
