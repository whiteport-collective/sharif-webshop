...
**Previous Step:** Ã¢â€ Â [01.1-Dimension Input](../01.1-dimension-input/01.1-dimension-input.md)
**Next Step:** Ã¢â€ â€™ [01.3-Product Detail](../01.3-product-detail/01.3-product-detail.md)
...
**Exit Points:**
- Taps a product card Ã¢â€ â€™ 01.3-Product Detail (overlay/expand)
- Taps "Edit" on dimension bar Ã¢â€ â€™ scrolls back to 01.1-Dimension Input
- Taps filter button Ã¢â€ â€™ filter overlay opens (brand, season)
...
**Related Pages:**
- [01.1-Dimension Input](../01.1-dimension-input/01.1-dimension-input.md) Ã¢â‚¬â€ previous step
- [01.3-Product Detail](../01.3-product-detail/01.3-product-detail.md) Ã¢â‚¬â€ next step
...
Horizontal card carousel below a sticky dimension summary bar. Part of the continuous parallax surface Ã¢â‚¬â€ not a separate page load.
...
+----------------------------------+
|                                  |
|  12 dekk funnet    [Filter Ã¢â€“Â¼]   |  Results Header
|                                  |
|  +----------+ +----------+ +--  |
|  | [img]    | | [img]    | |    |
|  |          | |          | |    |  Horizontal Card
...
|  +----------+ +----------+ +--  |
|                                  |
|  Ã¢â€”Â Ã¢â€”â€¹ Ã¢â€”â€¹ Ã¢â€”â€¹ Ã¢â€”â€¹ Ã¢â€”â€¹                    |  Scroll Indicator
|                                  |
+----------------------------------+
```
...
- **Desktop (>= 1024px):** Grid layout (3-4 cards per row), no swipe Ã¢â‚¬â€ standard scroll. Filter sidebar instead of collapsed overlay.
...
| Note | Identical to `dim-header` Ã¢â‚¬â€ shared global component |
...
| NO | "Endre" |
| EN | "Edit" |
| Behavior | onClick Ã¢â€ â€™ parallax scroll back to 01.1-Dimension Input |
...
| NO | "Filter" |
| EN | "Filter" |
| Behavior | onClick Ã¢â€ â€™ open filter overlay (brand, season checkboxes) |
| States | default, active (filters applied Ã¢â‚¬â€ show count badge) |
...
| Padding | space-md (horizontal Ã¢â‚¬â€ page-level, first card flush with content edge) |
...
| Purpose | Single tire product Ã¢â‚¬â€ all key info at a glance |
| Padding | space-lg |
| Element gap | space-sm |
...
| Behavior | onClick Ã¢â€ â€™ navigate to 01.3-Product Detail |
...
##### Ã¢â€ â€¢ `cards-v-space-sm` Ã¢â‚¬â€ compact gap between image and text content
...
##### Ã¢â€ â€¢ `cards-v-space-xs` Ã¢â‚¬â€ tight gap before EU label block
...
| Purpose | At-a-glance EU tire label ratings Ã¢â‚¬â€ fuel, grip, noise |
...
##### Ã¢â€ â€¢ `cards-v-space-sm` Ã¢â‚¬â€ breathing room before price
...
##### Ã¢â€ â€¢ `cards-v-space-xs` Ã¢â‚¬â€ tight gap before stock badge
...
| Variant | Color | NO | EN |
|---------|-------|----|----|
| In stock | success | "Pa lager" | "In stock" |
| Low stock | warning | "Fa igjen" | "Low stock" |
| Out of stock | text-muted | "Ikke pa lager" | "Out of stock" |
...
| Empty Ã¢â‚¬â€ No Results | Valid dimension but no products match | Illustration + "No tires found" message + suggestion to edit dimension | Tap edit, change filter |
| Error Ã¢â‚¬â€ API Failure | Network or server error | Error message with retry button | Tap retry, tap edit |
...
- Part of the continuous parallax surface Ã¢â‚¬â€ no page load between 01.1 and 01.2
...
- EU label mini-sliders are purely visual Ã¢â‚¬â€ no interaction, decorative ARIA role
- Product data fetched via API with dimension parameters; paginate if > 20 results
- Filter overlay (brand, season) is a separate component Ã¢â‚¬â€ not specified here
...
|---|----------|---------|--------|
| 1 | Card tap target Ã¢â‚¬â€ full card or dedicated button? | Full card is easier on mobile but may conflict with swipe gesture | Ã°Å¸â€Â´ Open |
| 2 | Show "out of stock" tires in results or hide them? | Showing them proves selection breadth but frustrates if many are unavailable | Ã°Å¸â€Â´ Open |
| 3 | Pagination strategy Ã¢â‚¬â€ infinite scroll, "load more", or fixed set? | Affects performance and scroll indicator design | Ã°Å¸â€Â´ Open |
| 4 | Season detection Ã¢â‚¬â€ auto-select based on current date? | Summer tires in June, winter in November Ã¢â‚¬â€ reduces one filter step | Ã°Å¸â€Â´ Open |
| 5 | Price display Ã¢â‚¬â€ include/exclude VAT indicator? | Norwegian convention is VAT-included, but B2B users may expect ex-VAT | Ã°Å¸â€Â´ Open |
