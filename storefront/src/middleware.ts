...
const DEFAULT_REGION = process.env.NEXT_PUBLIC_DEFAULT_REGION?.trim() || "us"
...
export async function middleware(request: NextRequest) {
  ...
  // URL already has country code — pass through (set cache cookie if missing)
  ...
  // No country code in URL — rewrite internally (URL stays clean in the browser)
  const rewritePath = pathname === "/" ? "" : pathname
  const url = request.nextUrl.clone()
  url.pathname = `/${countryCode}${rewritePath}`
